...
##1. Procédure correcte (étapes)
	...
	Éditer le fichier de configuration en utilisant un éditeur sécurisé (vim/nano) :
 	sudo nano /etc/ssh/sshd_config
	Appliquer les modifications en testant (optionnel) :
	Vérifier la syntaxe (OpenSSH ne fournit pas de commande syntax-check dédiée; on teste en lançant un sshd supplémentaire sur un port de test) :
 	sudo sshd -t || echo "sshd config OK"
	Ou démarrer une seconde instance test :
 	sudo /usr/sbin/sshd -f /etc/ssh/sshd_config -p 2222
	Redémarrer ou recharger le service ssh :
